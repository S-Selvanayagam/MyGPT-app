...
class AppViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun copyError() {
        ...
        clipboard.setPrimaryClip(ClipData.newPlainText("MyGPT", errorMessage()))
    }
    ...
    private fun loadAppConfig() {
        ...
        for (modelRecord in appConfig.modelList) {
            ...
            if (modelConfigFile.exists()) {
                Log.i("popopopo", "Load model config from ${modelConfigFile.absolutePath}")
                ...
                Log.i("popopop", "Download model config from ${modelRecord.modelUrl}")
                downloadModelConfig(
    ...
    private fun updateAppConfig(action: () -> Unit) {
        action()
        Log.i("popopop", "Update app config")
    ...
    private fun addModelConfig(modelConfig: ModelConfig, modelUrl: String, isBuiltin: Boolean) {
        ...
        Log.i("oooopopopopo", "Add model ${modelConfig.localId}")
        localIdSet.add(modelConfig.localId)
        Log.i("oooopopopopo", "App dir $appDirFile")
        modelList.add(
            ModelState(
                modelConfig,
                modelUrl,
    ...
    inner class ModelState(
        ...
        private fun switchToInitializing() {
            ...
            Log.i("popopop", "vandruchupo part 1 config from ${paramsConfigFile.absolutePath}")
            if (paramsConfigFile.exists()) {
                loadParamsConfig()
                switchToIndexing()
            } else {
                Log.i("popopop", "vandruchupo part 2 config from ${paramsConfigFile.absolutePath}")
        ...
        private fun loadParamsConfig() {
            ...
            Log.i("popopop", "vandruchupo config from ${paramsConfigFile.absolutePath}")
